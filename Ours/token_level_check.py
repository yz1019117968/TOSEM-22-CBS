...
from source.match import match, match_token, equal
...
def evaluateCorrectness_mixup(possibleMapping, streamPair, k=1):
    ...
    def tryAllPossible(possibleMapping, streamPair,matchLevel, k):
        ...
        indexes_token_rank = 0
        indexes_subtoken_rank = 0
        indexes_splited_rank = 0

        for x in possibleMapping:
            indexes_token_rank += 1
            indexes_subtoken_rank += 1
            indexes_splited_rank += 1
            if cnt >= 1:
                break
        ...
        return indexes_token_rank, indexes_subtoken_rank, indexes_splited_rank, cnt
    total_token_rank = 0
    total_subtoken_rank = 0
    total_splited_rank = 0
    for i in range(3):
        indexes_token_rank, indexes_subtoken_rank, indexes_splited_rank, cnt = tryAllPossible(possibleMapping, streamPair, matchLevel=i, k=k)
        total_token_rank += indexes_token_rank
        total_subtoken_rank += indexes_subtoken_rank
        total_splited_rank += indexes_splited_rank
        if cnt == 0:
            continue
        else:
            return total_token_rank, total_subtoken_rank, total_splited_rank, cnt
    return total_token_rank, total_subtoken_rank, total_splited_rank, 0

def get_cnt(fileInfo):
    ...
    streamPair = [buggyStream, fixedStream, src_desc, dst_desc, changed, modifiedMapping]

    possibleMapping = sortMapping(streamPair)
    total_token_rank, total_subtoken_rank, total_splited_rank, cnt = evaluateCorrectness_mixup(possibleMapping, streamPair)

    return max(total_token_rank, total_subtoken_rank, total_splited_rank), cnt
...
if __name__ == '__main__':
    ...
    # with open(f"./Dataset/{DATA_SET}_results.pkl", "rb") as fr:
    #     ys = pkl.load(fr)

    # print(stats.pointbiserialr(ys, cnt_list)
